...
<h1 class="text-center mt-2">View All My Products</h1>
<hr>
<br>
...
<form class="d-flex" action="{{ route('product.viewsearch') }}" method="GET">
  ...
  <button class="btn btn-outline-success" type="submit">Search</button>
    ...
    <div class="row">
    @foreach ($showallmyproducts as $product)
            ...
            <!-- <img src="images/{{ $product->picture }}" class="card-img-top" alt="..."> -->
                ...
                <div class="card-body">
                  ...
                  <form action="{{ route('product.delete', $product->id)}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <pre><a href="{{ route('product.edit', $product->id) }}" class="btn btn-success">Edit</a> <button type="submit" class="btn btn-danger">Delete</button></pre>
                  </form>
                  &emsp;&nbsp;<a href="{{ route('product.show', $product->id) }}" class="btn btn-primary">View Details</a>
                </div>
              </div>
